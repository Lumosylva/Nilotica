...
def main():
    ...
    setup_logging(service_name=f"MarketGatewayRunner[{args.env}]", level=args.log_level.upper())
    ...
    logger.info(f"正在初始化行情网关服务(RPC模式) for environment: [{args.env}]...")
    try:
        ...
        logger.info(f"行情网关服务(RPC模式) for [{args.env}] 已退出。")
