...
class StrategyEngine: # Renamed class
    def __init__(self, gateway_pub_url: str, order_req_url: str, order_report_url: str, strategies_config: Dict[str, Dict[str, Any]]):
        ...
        logger.info(f"策略引擎连接订单网关: {order_req_url}")
        # --- End ZMQ Setup ---
        ...
        for strategy_name, config_data in strategies_config.items():
            ...
            try:
                ...
                if '.' not in vt_symbol:
                     logger.error(f"策略 '{strategy_name}' 的 vt_symbol '{vt_symbol}' 格式无效，应为 'symbol.exchange'。跳过加载。")
                     continue
                ...
                if not issubclass(strategy_class, BaseLiveStrategy):
                     logger.error(f"类 '{class_path}' 不是 BaseLiveStrategy 的子类。跳过加载策略 '{strategy_name}'。")
                     continue
                ...
                if vt_symbol in self.symbol_strategy_map:
                     ...
                     logger.warning(f"vt_symbol '{vt_symbol}' 已被其他策略 '{self.symbol_strategy_map[vt_symbol].strategy_name}' 注册。策略 '{strategy_name}' 也将监听此合约。")
                ...
                logger.info(f"策略 '{strategy_name}' (类: {class_name}, 合约: {vt_symbol}) 加载成功。")

            except ImportError:
                logger.exception(f"导入策略类 '{class_path}' 失败。跳过加载策略 '{strategy_name}'。")
            except AttributeError:
                 logger.exception(f"在模块 '{module_path}' 中未找到策略类 '{class_name}'。跳过加载策略 '{strategy_name}'。")
            except KeyError as e:
                logger.error(f"策略 '{strategy_name}' 的配置缺少必要键: {e}。跳过加载。")
            except Exception as e:
                logger.exception(f"加载策略 '{strategy_name}' 时发生未知错误: {e}")
        logger.info(f"策略加载完成。共加载 {len(self.strategies)} 个策略实例。")
        ...
        logger.info(f"  订阅通用主题前缀: {topic_map['order']}*, {topic_map['trade']}*, {topic_map['account']}*, {topic_map['log']}, {topic_map['heartbeat_ordergw']}")
    ...
    def _send_rpc_request(self, request_tuple: tuple, timeout_ms: int = RPC_TIMEOUT_MS, retries: int = RPC_RETRIES) -> Any | None:
        ...
        if not hasattr(self, 'order_requester') or not self.order_requester or self.order_requester.closed:
             logger.error("Order requester socket is not available or closed.")
            ...
            try:
                    ...
                    try:
                        ...
                        if topic_str == "heartbeat.ordergw":
                            ...
                            if isinstance(data_obj, TickData):
                                ...
                                logger.warning(f"Received non-TickData object on tick topic {topic_str}: {type(data_obj)}")
                ...
                if self.last_ordergw_heartbeat_time > 0 and \
                   (current_time - self.last_ordergw_heartbeat_time > HEARTBEAT_TIMEOUT_S):
                    if self.gateway_connected: # Only log error once when connection is lost
                        logger.error(f"订单网关心跳超时 ({HEARTBEAT_TIMEOUT_S:.1f}s)! 标记为断开连接。 (Last HB: {datetime.fromtimestamp(self.last_ordergw_heartbeat_time).isoformat()})")
                        self.gateway_connected = False
                ...
                if self._is_trading_hours():
                 ...
                 if not self.running or err.errno == zmq.ETERM:
                     logger.info(f"ZMQ 错误 ({err.errno}) 发生在服务停止期间，正常退出。")
                     break
                 else:
    ...
    def process_tick(self, vt_symbol: str, tick: TickData):
        ...
        strategy = self.symbol_strategy_map.get(vt_symbol)
        if strategy:
            # logger.debug(f"Dispatching tick for {vt_symbol} to strategy {strategy.strategy_name}")
            try:
                strategy.on_tick(tick)
            except Exception as e:
                 logger.exception(f"策略 {strategy.strategy_name} 在 on_tick 处理 {vt_symbol} 时出错: {e}")
        # else: # Log if no strategy found? Can be noisy.
             # logger.debug(f"No strategy registered for tick symbol: {vt_symbol}")
    ...
    def process_order_update(self, order: OrderData):
        ...
        strategy = self.symbol_strategy_map.get(vt_symbol)
        if strategy:
            # logger.debug(f"Dispatching order update {order.vt_orderid} for {vt_symbol} to strategy {strategy.strategy_name}")
             try:
        ...
        # else: # May receive orders for symbols not actively managed by strategies here
             # logger.debug(f"No strategy registered for order symbol: {vt_symbol}")
    ...
    def process_trade_update(self, trade: TradeData):
        ...
        logger.info(f"收到成交回报: {trade.vt_symbol} OrderID: {trade.vt_orderid} Offset: {trade.offset.value if hasattr(trade,'offset') else trade.offset} Price: {trade.price} Vol: {trade.volume}")
        self.trades.append(trade) # Optional: store globally

        strategy = self.symbol_strategy_map.get(vt_symbol)
        if strategy:
            # logger.debug(f"Dispatching trade update {trade.vt_tradeid} for {vt_symbol} to strategy {strategy.strategy_name}")
            try:
                strategy.on_trade(trade)
            except Exception as e:
                logger.exception(f"策略 {strategy.strategy_name} 在 on_trade 处理 {trade.vt_tradeid} 时出错: {e}")
        # else:
             # logger.debug(f"No strategy registered for trade symbol: {vt_symbol}")
    ...
    def process_account_update(self, account: AccountData):
        ...
        # logger.info(f"账户更新: ID={accountid}, Balance={balance:.2f}, Available={available:.2f}")
