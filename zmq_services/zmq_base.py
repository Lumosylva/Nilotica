...
class ZmqPublisherBase:
    ...
    def start(self, pub_address: str) -> bool:
        ...
        """
        if self._active:
            logger.warning(_("{} 已在运行中。").format(self.__class__.__name__))
            return False

        logger.info(_("正在启动 {}...").format(self.__class__.__name__))
        try:
            ...
            # logger.info(_("设置 ZMQ PUB socket SNDHWM: {}").format(sndhwm))

            self._socket_pub.bind(pub_address)
            self._pub_address = pub_address
            self._active = True
            logger.info(_("{} 已启动，PUB socket 绑定到 {}").format(self.__class__.__name__, pub_address))
            return True
        except zmq.ZMQError as e:
            logger.exception(_("{} 启动失败 (绑定 PUB socket 到 {} 时出错): {}").format(
            ...
            if self._socket_pub:
                try:
                    ...
                    logger.error(_("关闭 ZMQ PUB socket 时: {}").format(ex))
                    pass
                self._socket_pub = None
            return False
        except Exception as e:
             logger.exception(_("{} 启动时发生未知错误: {}").format(self.__class__.__name__, e))
             self._active = False
             if self._socket_pub:
                 try:
                     self._socket_pub.close()
    ...
    def stop(self):
        ...
        if not self._active:
            logger.debug(_("{} 未运行。").format(self.__class__.__name__))
            return

        logger.info(_("正在停止 {}...").format(self.__class__.__name__))
        ...
        if self._socket_pub:
            ...
            try:
                ...
                logger.info(_("ZMQ PUB socket 已关闭。"))
            except Exception as e:
                logger.error(_("关闭 ZMQ PUB socket 时出错: {}").format(e))
        ...
        try:
            ...
            if self._context and not self._context.closed:
                self._context.term()
                logger.info(_("ZMQ Context 已终止。"))
        except Exception as e:
            logger.error(_("终止 ZMQ Context 时出错: {}").format(e))
    ...
    def publish(self, topic: str, data: object) -> bool:
        ...
        if not self._active or not self._socket_pub:
            logger.warning(_("发布失败：{} 未激活或 PUB socket 不可用。 Topic: {}").format(
        ...
        try:
            ...
            with self._lock:
                ...
                if not self._active or not self._socket_pub:
                    logger.warning(_("发布取消：在获取锁后发现服务已停止。Topic: {}").format(topic))
                    return False
                # +++ Add specific logging for tick sending +++
                send_attempted = False
                send_succeeded = False
                if topic.startswith("tick."):
                    logger.debug(_("[PUB Tick Send] Attempting send_multipart for Topic: {}").format(topic))
                    send_attempted = True
                # +++ End Add +++
                self._socket_pub.send_multipart([topic_bytes, data_bytes])
                # +++ Log success for tick +++
                if send_attempted:
                    logger.debug(_("[PUB Tick Send] send_multipart call completed for Topic: {}").format(topic))
                    send_succeeded = True # Assume success if no exception
                # +++ End Log +++
            # --- Adjust general publish log level to DEBUG --- 
            # logger.debug(_("已发布数据 (主题: {}, 大小: {} B)").format(topic, len(data_bytes))) # Keep as DEBUG
            # --- End Adjust ---
            return True
        except (msgpack.PackException, TypeError) as e_msgpack:
            logger.error(_("Msgpack 序列化错误 (发布数据, 主题: {}): {}").format(topic, e_msgpack)) # Renamed
            try:
                # Log the type of the *original* data and the *converted* data
                logger.error(_("原始对象类型: {}").format(type(data)))
                logger.error(_("转换后对象类型: {}").format(type(data_to_pack)))
            except Exception as log_err:
                 logger.error(_("记录对象细节时出错: {}").format(log_err))
            return False
        except zmq.ZMQError as e_zmq:
            # 处理特定 ZMQ 错误，例如缓冲区满 (EAGAIN)
            if e_zmq.errno == zmq.EAGAIN:
                logger.warning(_("发布事件时 ZMQ 缓冲区可能已满 (EAGAIN). Topic: {}").format(topic))
            else:
                logger.error(_("发布事件时 ZMQ 错误: {}. Topic: {}").format(e_zmq, topic))
            return False
        except Exception as e:
            logger.exception(_("发布事件时发生未知错误 (主题: {}): {}").format(topic, e))
            return False 