...
logging.basicConfig(level=logging.INFO, format="%(asctime)s - HatchBuildTTS - %(levelname)s - %(message)s")
...
class CustomBuildHookTTS(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: Dict[str, Any]) -> None:
        ...
        if system == "Linux":
            specific_settings = {
                "library_dirs": [
                    str(root_path / d) for d in library_dirs_rel_base
                ],  # Tell linker where to find .so for linking (if needed, usually not for .so)
                "extra_compile_args": [
                    "-std=c++17",
                    "-O3",
                ...
                "runtime_library_dirs": [
                    "$ORIGIN"
                ],  # Linker searches in the same directory as the extension at runtime
            }
        elif system == "Windows":
            specific_settings = {
                "library_dirs": [
                    str(root_path / d) for d in library_dirs_rel_base + library_dirs_rel_win
                ],  # Tell linker where to find .lib files
                "extra_compile_args": [
                    "/O2",
                    "/MT",
                    "/EHsc",
                    ...
                    "/std:c++17",
                    "/wd4100",
                ],  # Specify minimum Windows version (e.g., 0x0A00 for Win 10)
                "extra_link_args": [],
                "runtime_library_dirs": [],
            }
        else:
            ...
            if stubgen_cmd_base and artifact_copied:
                ...
                try:
                    ...
                    result = subprocess.run(
                        ...
                        check=True,
                        env=stubgen_env,
                        cwd=artifact_dest_dir_abs,  # CWD is key for DLL finding on Windows
                        capture_output=True,
                        text=True,
                        encoding="gbk",
                        errors="ignore",
                    ...
                    if stub_file.exists():
                        ...
                        if result.stdout:
                            logger.debug(f"Stubgen STDOUT ({full_module_name}):\\n{result.stdout}")
                        if result.stderr:
                            logger.debug(f"Stubgen STDERR ({full_module_name}):\\n{result.stderr}")
                    else:
                        logger.warning(
                        ...
                        if result.stderr:
                            logger.warning(f"Stubgen STDERR ({full_module_name}):\\n{result.stderr}")
                    ...
                    logger.error(
                        f"An unexpected error occurred during stub generation for {module_name}: {stub_exc}",
                        exc_info=True,
                    )
