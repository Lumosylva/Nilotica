# Nilotica, a futures quantitative trading system based on vnpy
...
The project uses `hatch` to compile `.pyd` market and transaction files in the `vnpy_ctp\api\` path and uses `pybind11-stubgen` to generate their corresponding `.pyi` stub files.
...
├── hatch_build.py - Automated compilation script that compiles CTP C++ interface files into Python-callable pyd and pyi files.
...
├── run.bat - One-click start of market gateway, order execution gateway, strategy subscriber, risk control management, data recording script
